...
func (cum *CsvUtilMgr) getFieldMap(config interface{}) map[string][]string {
...
// readCsv 读取csv文件
func (cum *CsvUtilMgr) readCsv(fileName string) [][]string {
...
func (cum *CsvUtilMgr) getTagMap(fields []string) map[int]string {
...
func (cum *CsvUtilMgr) getValueType(SlicePtr interface{}) reflect.Type {
...
// 指针只能取interface{}, 值得取Elem,否则会崩
// tagMap: csv第一行
// filedMap: key: tag
func (cum *CsvUtilMgr) genConfig(dataPtr interface{}, csvData [][]string, tagMap map[int]string,
	...
	outInnerType := cum.getValueType(dataPtr)
...
func (cum *CsvUtilMgr) trimNumber(s string) string {
...
// LoadCsv 加载csv配置
func (cum *CsvUtilMgr) LoadCsv(fileName string, SlicePtr interface{}) {
	csvFile := fileName + ".csv"
	csvData := cum.readCsv(csvFile)
	if len(csvData) <= 1 {
	...
	err := cum.ParseDataSimple(csvData, SlicePtr, fileName)
	if err != nil {
		fmt.Println(err.Error())
		return
	}
...
func (cum *CsvUtilMgr) LoadEventsCsv(fileName string, SlicePtr interface{}) {
...
func (cum *CsvUtilMgr) getFieldMapSimple(config interface{}, tagMap map[int]string) (map[string][]string, map[int]bool, string) {
	t := reflect.TypeOf(config)
	fieldMap := make(map[string][]string, t.NumField())
	...
	for i := 0; i < t.NumField(); i++ {
		...
		for key, v := range tagMap {
			if (trim == "" || trim == "1") && tag == cum.trimNumber(v) {
				trimFlag[key] = true
			}
		}
...
func (cum *CsvUtilMgr) ParseDataSimple(csvData [][]string, dataPtr interface{}, fileName string) (err error) {
	outInnerType := cum.getValueType(dataPtr)
	data := reflect.New(outInnerType.Elem())

	value := reflect.Indirect(data)
	tagMap := cum.getTagMap(csvData[0])
	fieldMap, trimFlag, keyTag := cum.getFieldMapSimple(value.Interface(), tagMap)
	err = cum.genConfig(dataPtr, csvData, tagMap, fieldMap, trimFlag, fileName, keyTag)
